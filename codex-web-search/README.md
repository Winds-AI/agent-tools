...
- `web_search` — search the public web via OpenAI's Responses API (`web_search` tool) through the Codex ChatGPT backend, returning a concise summary with sources.
...
pi install ./pi-extensions/codex-web-search
```

Restart Pi after installation.

To use the extension directly without installing, copy `index.ts` (and its sibling `auth.ts`, `constants.ts`, `search.ts`, `types.ts`) into `~/.pi/agent/extensions/`.
